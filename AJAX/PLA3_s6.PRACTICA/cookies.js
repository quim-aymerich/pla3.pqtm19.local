 ...
 * 
 * @param string name Nombre de la Cookie
 * @returns boolean Falso si no existe la cookie
 */
...
document.querySelector("#cookiePanel button").onclick=function(){
      
      createCookie("cOk","ok",1,"/",""); // (nombre,valor,expire,path,domain )
      window.sessionStorage.setItem("sOk","ok");
      document.getElementById("cookiePanel").style.display="none";
}
    
document.getElementById("spanCookiePanel").onclick=function(){
      document.getElementById("cookiePanel").style.display="none";
}
    
    var cOk=getCookie("cOk");
    var sOk = window.sessionStorage.getItem("sOk");
    if(cOk=="ok" && sOk=="ok"){
      document.getElementById("cookiePanel").style.display="none";
}